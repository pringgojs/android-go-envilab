...
class ClientAddViewModel @Inject constructor(private val repository: Repository): BaseViewModel<CompanyResponse>() {

    private val compositeDisposable by lazy { CompositeDisposable() }

    override fun getDisposable(): CompositeDisposable? = compositeDisposable

    fun postCompanyAdd(companyAddRequest: CompanyAddRequest) {
        showLoading()
        repository.postCompanyAdd(companyAddRequest)
