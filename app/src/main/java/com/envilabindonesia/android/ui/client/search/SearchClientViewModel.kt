...
class SearchClientViewModel @Inject constructor(private val repository: Repository): BaseViewModel<BasePageResponse<CompanyResponse>>() {

    private val compositeDisposable by lazy { CompositeDisposable() }

    override fun getDisposable(): CompositeDisposable? = compositeDisposable

    fun getCompanies(fromRow: Int, keyword: String = "") {
        showLoading()
        repository.getCompanies(fromRow, keyword)
